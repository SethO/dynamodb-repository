...
module.exports = class HashRepository {
   ...
   */
  constructor({ tableName, keyName, idOptions, documentClient = new DynamoDB.DocumentClient() }) {
  ...
  async update(item) {
    ...
    const { revision: previousRevision, createdAt } = item;
    const putParams = {
      TableName: this.tableName,
      Item: itemToSave,
      ConditionExpression:
        'attribute_exists(#key) AND revision = :prevRev AND createdAt = :createdAt',
